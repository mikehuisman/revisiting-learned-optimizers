...
from .modules.shell import Input, INP_TO_CODE, TYPE_TO_SIZE
...
class LSTM(Algorithm):
    ...
    def __init__(self, grad_clip, second_order, hidden_size, input_type, **kwargs):
        """
        Parameters
        ----------
        grad_clip : float
        ...
        **kwargs : dict
        ...
        self.input_size = TYPE_TO_SIZE[input_type]
        self.hidden_size = hidden_size
        self.second_order = second_order
        self.input_type = INP_TO_CODE[input_type]
        ...
        # put baselearner parameters in the "place" :)
        flat_baselearner_params = torch.cat([p.reshape(-1).clone().detach().to(self.dev) for p in self.baselearner.parameters()], 0)
        self.metalearner.metalstm.init_cI(flat_baselearner_params)


        self.initialization = [p.clone().detach().to(self.dev) for p in self.metalearner.parameters()]
        for p in self.initialization:
            p.requires_grad = True

        # Set up loss, optimizer, learning rate scheduler
        self.optim = self.opt_fn(self.initialization, self.lr)
    ...
    def train_learner(self, train_x, train_y, train_mode=True):
        ...
        cI = self.initialization[-3].clone()
        ...
        for _ in range(self.T):
            for i in range(0, len(train_x), batch_size):
                ...
                grad = torch.cat([p.reshape(-1) / batch_size for p in grad], 0)

                if self.input_type == Input.ProcLossGrads:
                ...
                elif self.input_type == Input.RawGrads:
                    metalearner_input = grad.unsqueeze(1)
                elif self.input_type == Input.ProcGrads:
                    metalearner_input = [preprocess_grad_loss(grad), grad.unsqueeze(1)]

                cI, h = self.metalearner.forward_weights(metalearner_input, hs=hs[-1], weights=self.initialization)
                hs.append(h)
        return cI
    ...
    def train(self, train_x, train_y, test_x, test_y):
        ...
        cI = self.train_learner(train_x, train_y) # fast weights
        unflattened_cI = unflatten_weights(cI, self.baselearner.parameters())
        ...
        if self.grad_clip == 10:
            for p in self.initialization:
                p.grad = torch.clamp(p.grad, -self.grad_clip, +self.grad_clip)
        else:
            nn.utils.clip_grad_norm_(self.initialization, self.grad_clip)
        self.optim.step()
