...
class MetaLSTMCell(nn.Module):
    ...
    def forward_weights(self, inputs, hx=None, weights=None):
        WF, WI, cI, bI, bF = weights 
        x_all, grad = inputs
        batch, _ = x_all.size()

        if hx is None:
            f_prev = torch.zeros((batch, self.hidden_size), device=self.WF.device)
            i_prev = torch.zeros((batch, self.hidden_size), device=self.WF.device)
            c_prev = cI
            hx = [f_prev, i_prev, c_prev]
        ...
        # f_t = sigmoid(W_f * [grad_t, loss_t, theta_{t-1}, f_{t-1}] + b_f)
        f_next = torch.mm(torch.cat((x_all, c_prev, f_prev), 1), WF) + bF.expand_as(f_prev)
        # i_t = sigmoid(W_i * [grad_t, loss_t, theta_{t-1}, i_{t-1}] + b_i)
        i_next = torch.mm(torch.cat((x_all, c_prev, i_prev), 1), WI) + bI.expand_as(i_prev)
        # next cell/params
        c_next = torch.sigmoid(f_next).mul(c_prev) - torch.sigmoid(i_next).mul(grad)
...
class MetaLearner(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, n_learner_params):
        ...
        self.lstm = LSTMCell(input_size=input_size, hidden_size=hidden_size, bias=True)
    ...
    def forward(self, inputs, hs=None):
        ...
        lstmhx, lstmcx = self.lstm(inputs, hs[0]) #hs[0] is a tuple (hx, cx) = (hidden state, cell state)
    ...
    def forward_weights(self, inputs, hs=None, weights=None):
        """Perform a single unroll step of the Meta LSTM
        ...
        lstmhx, lstmcx = self.lstm.forward_weights(x=inputs, hidden=hs[0], weights=weights[0:4]) #hs[0] is a tuple (hx, cx) = (hidden state, cell state)
        flat_learner_unsqzd, metalstm_hs = self.metalstm.forward_weights(inputs=[lstmhx, grad], hx=hs[1], weights=weights[4:])
